...
class AuthenticationTest < ActionDispatch::IntegrationTest
  ...
  test "non-designer cannot access layout management" do
    login_as :non_admin
    get admin_layouts_path
    assert_redirected_to admin_pages_path
  end

  test "each role can login successfully" do
    [:admin, :designer, :existing, :non_admin].each do |role|
      get logout_path rescue nil
      user = users(role)
      post login_post_path, params: {username_or_email: user.login, password: "password"}
      assert_redirected_to welcome_path, "#{role} should be able to login"
      follow_redirect!
    end
  end
end
